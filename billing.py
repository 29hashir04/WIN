...
class BillClass:
    def __init__(self, root):
        ...
        self.root.title("WorkForce Information Network ----- A DBMS PROJECT by Eesha, Hashir and Shajia")
        ...
        title = Label(self.root, text="Workforce Information Network", image=self.icon_title, compound=LEFT,
                      font=("times new roman", 40, "bold"), bg="#010c48", fg="white", anchor="w", padx=20).place(x=0, y=0, relwidth=2, height=70)

        # button_logout
        btn_logout = Button(self.root, command=self.logout, text="Logout", font=("times new roman", 15, "bold"),
        ...
        self.lbl_clock = Label(root,
                               text=f"Welcome to Workforce Information Network \t\t Date: {current_date} \t\tTime: {current_time}",
        ...
        pTitle = Label(projectFrame1, text="All Projects", font=("goudy old style", 20, "bold"), bg="#262626",
        ...
        lbl_search = Label(projectFrame2, text="Search Project | By Name ", font=("times new roman", 15, "bold"),
                           bg="white", fg="green").place(x=2, y=5)

        lbl_search = Label(projectFrame2, text="Project Name", font=("times new roman", 15, "bold"), bg="white").place(
            x=2, y=45)

        txt_search = Entry(projectFrame2, textvariable=self.var_search, font=("times new roman", 15),
                           bg="lightyellow").place(x=128, y=47, width=150, height=22)
        btn_search = Button(projectFrame2, command=self.search, text="search", font=("goudy old style", 15),
                            bg="#2196f3", fg="white", cursor="hand2").place(x=285, y=45, width=100, height=25)
        btn_show_all = Button(projectFrame2, command=self.show, text="Show All", font=("goudy old style", 15),
        ...
        self.project_Table = ttk.Treeview(projectFrame3, columns=("pid", "name", "stipend", "length", "status"),
        ...
        lbl_notes = Label(projectFrame1, text="Note:'Enter 0 Quantity to remove product from the Cart'",
        ...
        cTitle = Label(ClientFrame, text="Client Details", font=("goudy old style", 15), bg="lightgray").pack(side=TOP,
                                                                                                              fill=X)
        lbl_name = Label(ClientFrame, text="Name", font=("times new roman", 15), bg="white").place(x=5, y=35)
        txt_name = Entry(ClientFrame, textvariable=self.var_cname, font=("times new roman", 13),
                         bg="lightyellow").place(x=80, y=35, width=180)

        lbl_contact = Label(ClientFrame, text="Contact No.", font=("times new roman", 15), bg="white").place(x=270,
                                                                                                             y=35)
        txt_contact = Entry(ClientFrame, textvariable=self.var_contact, font=("times new roman", 13),
        ...
        btn_7 = Button(cal_Frame, command=lambda: self.get_input(7), text='7', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=1, column=0)
        btn_8 = Button(cal_Frame, command=lambda: self.get_input(8), text='8', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=1, column=1)
        btn_9 = Button(cal_Frame, command=lambda: self.get_input(9), text='9', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=1, column=2)
        btn_sum = Button(cal_Frame, command=lambda: self.get_input('+'), text='+', font=('arial', 15, 'bold'), bd=5,
                         width=4, pady=10, cursor="hand2").grid(row=1, column=3)

        btn_4 = Button(cal_Frame, command=lambda: self.get_input(4), text='4', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=2, column=0)
        btn_5 = Button(cal_Frame, command=lambda: self.get_input(5), text='5', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=2, column=1)
        btn_6 = Button(cal_Frame, command=lambda: self.get_input(6), text='6', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=2, column=2)
        btn_sub = Button(cal_Frame, command=lambda: self.get_input('-'), text='-', font=('arial', 15, 'bold'), bd=5,
                         width=4, pady=10, cursor="hand2").grid(row=2, column=3)

        btn_1 = Button(cal_Frame, command=lambda: self.get_input(1), text='1', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=3, column=0)
        btn_2 = Button(cal_Frame, command=lambda: self.get_input(2), text='2', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=3, column=1)
        btn_3 = Button(cal_Frame, command=lambda: self.get_input(3), text='3', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=10, cursor="hand2").grid(row=3, column=2)
        btn_sum = Button(cal_Frame, command=lambda: self.get_input('*'), text='*', font=('arial', 15, 'bold'), bd=5,
                         width=4, pady=10, cursor="hand2").grid(row=3, column=3)

        btn_0 = Button(cal_Frame, command=lambda: self.get_input(0), text='0', font=('arial', 15, 'bold'), bd=5,
                       width=4, pady=15, cursor="hand2").grid(row=4, column=0)
        btn_c = Button(cal_Frame, command=self.clear, text='C', font=('arial', 15, 'bold'), bd=5, width=4, pady=15,
                       cursor="hand2").grid(row=4, column=1)
        btn_eq = Button(cal_Frame, command=self.perform_cal, text='=', font=('arial', 15, 'bold'), bd=5, width=4,
                        pady=15, cursor="hand2").grid(row=4, column=2)
        btn_div = Button(cal_Frame, command=lambda: self.get_input('/'), text='/', font=('arial', 15, 'bold'), bd=5,
        ...
        self.cartTitle = Label(cart_Frame, text="Cart \t Total Projects: [0]", font=("goudy old style", 15),
                               bg="lightgray")
        ...
        self.cartTable = ttk.Treeview(cart_Frame, columns=("pid", "name", "stipend", "length", "status"),
                                      yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM, fill=X)
        scrolly.pack(side=RIGHT, fill=Y)
        ...
        lbl_p_stipend = Label(Add_CartWidgetsFrame, text="Price per Projects", font=("times new roman", 15),
                              bg="white").place(x=230, y=5)
        txt_p_name = Entry(Add_CartWidgetsFrame, textvariable=self.var_stipend, font=("times new roman", 15),
                           bg="lightyellow", state="readonly").place(x=230, y=35, width=150, height=22)

        lbl_p_qty = Label(Add_CartWidgetsFrame, text="Project Length", font=("times new roman", 15), bg="white").place(
            x=390, y=5)
        txt_p_qty = Entry(Add_CartWidgetsFrame, textvariable=self.var_length, font=("times new roman", 15),
        ...
        btn_clear_cart = Button(Add_CartWidgetsFrame, command=self.clear_cart, text="Clear",
                                font=("times new roman", 15, "bold"), bg="lightyellow", cursor="hand2").place(x=180,
                                                                                                              y=70,
                                                                                                              width=150,
                                                                                                              height=30)
        ...
        BTitle = Label(billFrame, text="Client Bill Area", font=("goudy old style", 20, "bold"), bg="#f44336",
                       fg="white").pack(side=TOP, fill=X)
        ...
        btn_print = Button(billMenuFrame, command=self.print_bill, text="Print", cursor="hand2",
                           font=("goudy old style", 15, "bold"), bg="lightgreen", fg="white")
        btn_print.place(x=2, y=80, width=120, height=50)

        btn_clear_all = Button(billMenuFrame, command=self.clear_all, cursor="hand2", text="Clear All",
                               font=("goudy old style", 15, "bold"), bg="gray", fg="white")
        btn_clear_all.place(x=124, y=80, width=120, height=50)

        btn_generate = Button(billMenuFrame, command=self.generate_bill, cursor="hand2", text="Generate Bill/Save Bill",
                              font=("goudy old style", 10, "bold"), bg="#009688", fg="white")
        btn_generate.place(x=246, y=80, width=160, height=50)

        footer = Label(self.root,
                       text="WIN-Workforce Information Network \nFor any Technical Issue contact: 03320208649",
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "":
                ...
                cur.execute(
                    "select pid, name, stipend, length, status from project where name LIKE '%" + self.var_search.get() + "%' and status = 'Active'")
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.project_Table.delete(*self.project_Table.get_children())
                    for row in rows:
    ...
    def add_update_cart(self):
        if self.var_pid.get() == '':
            ...
            cart_data = [self.var_pid.get(), self.var_pname.get(), price_cal, self.var_length.get(),
                         self.var_status.get()]
            # update cart
            present = 'no'
            index_ = 0
